...
* @Last Modified time: 2017-10-29 23:32:33
*/
...
function Game(){
  ...
  Game.prototype={
    ...
    getChar:function(){
    	...
    	let tops=Math.floor(Math.random()*100+50);
    	let lefts=Math.floor(Math.random()*(chwidth-400)+100);
		while(this.checkPosition(lefts)){
           lefts=Math.floor(Math.random()*(chwidth-400)+100);
	   }
    				...
    				if(that.hp<=0){
    					let flag=confirm('游戏结束，你要重新开始吗？')
    						if(flag){
    							...
    							location.replace('../sense/index.html');
    						}
